   ...
   public function insert (Request $request){
    try {
        $request->validate([
            ...
            'identity' => 'string|unique:additional_information',
            'npwp' => 'string|unique:additional_information',
            'customer_type' => 'integer|in:0,1',
        ]);

        $CodeProduct = Product::where('id','2')->first();
        ...
        AdditionalData::create([
         ...
         $val_day = $diff->days;
         $selisih = $val_day - 365;
         $MainRate = MasterRate::where('rate_code','FL-01')->first();
         $percent = $pertanggungan*($MainRate['rate']/100);
         $test =  (20/100)*$pertanggungan*($MainRate['rate']/100);
         $total = $test+$percent;
         echo '<br> FL-01 :' . $total;   
